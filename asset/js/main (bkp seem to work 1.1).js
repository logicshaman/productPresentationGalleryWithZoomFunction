...
function prdctPrsnttonGlry_E(){
	...
	for (var i = optimumImgNums - 1; i >= 0; i--) {
		...
		e_actualSize_jQ.prepend("<div class='z "+activateBydefault()+" otimumImg_div_Z otimumImg_div_"+(i+1)+"'><img class='optimumImg"+(i+1)+"' src="+imgLctn+imgOptimum[i]+">");
	...
	function imageZoom(imgID, resultID) {
		...
		for (var i = 0; i < optimumImgNums; i++) {
			console.log(i);
			img = document.getElementById(imgID).getElementsByClassName("otimumImg_div_Z")[i].firstChild;
			...
			function moveLens(e) {
				...
				result.style.backgroundPosition = "-" + (x*0.77) + "px -" + (y*0.725) + "px"; // Display what the lens "sees". EDIT THE VALUES ACCORDINGLY
			}
		...
		// 	return {x : x, y : y};
		// }
	}imageZoom("myimage", "zoomResult"); // call and assign targets
	...
	$("#zoomResult").css({
		"width": imgToZoomW*1.5+"px",
		"height": imgToZoomH*1.5+"px"
	})
